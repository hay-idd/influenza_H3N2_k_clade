...
####function to generate dault incidence from weekly incidence data 
...
#remove the rt estimates that don't look weird 
...
peak_dates <- Rt_df %>%
  ...
  group_by(season_label) %>%
  slice_max(order_by = Rt_mean, n = 1) %>%
  select(season_label, peak_date = date, peak_Rt = Rt_mean)
...
p_Rt_shifted_1 <- ggplot(Rt_df_shifted, aes(x = days_since_peak, y = Rt_mean, color = pandemic_phase)) +
 ...
 # xlim(c(-50,50))+
  ...
  geom_text(data = peak_dates, aes(x = 35, y = 1.5, label = paste0(round(peak_Rt, 2))), 
...
p_Rt_shifted_2 <- ggplot(Rt_df_shifted, aes(x = days_since_peak, y = Rt_mean,
  ...
  xlim(c(-50, 50)) +
  geom_vline(xintercept = 0, linetype = "dotted", color = "red") +
  geom_text(data = peak_dates, aes(x = 5, y = 2.5, label = paste0(round(peak_Rt, 2))), 
            inherit.aes = FALSE,    color = "red",     vjust = -0.5,   size = 3  ) +
  # Add text labels for peak Rt  
...
ggsave("rt_estimates_plots/pre-Christmas_rt_estimates_by_season_staked_1_befire_chrismas_day_no_limit_on_axis.png",p_Rt_shifted_stacked_1,height =8 ,width = 8)
