...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("alpha2", "Relative susceptibility susceptible class", value = 0.6, step = 0.05),
      numericInput("R0", "Basic reproductive number R0", value = 2, step = 0.01),
      numericInput("gamma", "Infectious period (days) gamma", value = 4.8, step = 0.1),
      ...
      sliderInput("sim_start_date", "Simulation start date",
      ...
      sliderInput("prop_all_contacts_christmas", "Multiplier for all contacts in Christmas period",
                  min = 0.5, max = 3, value = 1.2, step = 0.01),
      hr(),
...
server <- function(input, output, session) {
  ...
  run_model <- eventReactive(input$run_model, {
    ...
    tryCatch({
      ...
      contacts_overall <- bind_rows(contacts_others, contacts_schools, contacts_work, contacts_home) %>% sample_frac(prop_all_contacts_christmas, replace = TRUE)
      ...
      contacts_home <- contacts %>% filter(cnt_home == 1) %>% sample_frac(prop_home_contacts_in_christmas, replace = TRUE)
  ...
  output$inc_plot <- renderPlot({
    ...
    flu_dat1 <- flu_dat %>% filter(date >= res$meta$start_date,date <= end_date)
  ...
  output$cum_table <- renderTable({
    ...
    if (is.null(df)) return(data.frame(Message = "No cumulative_incidence returned by model"))
    
    # optional: format numeric columns for readability
    numcols <- sapply(df, is.numeric)
    df[numcols] <- lapply(df[numcols], function(x) {
      # round numeric columns reasonably
      ifelse(abs(x) < 1, signif(x,3), round(x,0))
    })
    
    # hide rownames, show columns as is
    df
