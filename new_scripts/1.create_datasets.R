...
## Demography data
age_cap <- 90L
age_groups <- read_csv("new_data/ons_ages_2024.csv")
...
ili_cases_recent <- read_csv("new_data/rcgp_ili_2023_2025.csv") %>% fill(Year,.direction="down") %>% arrange(Year,Week) %>% drop_na()
ili_cases_older <- read_csv("new_data/rcgp_ili_2021_2023.csv")%>% 
...
write_csv(ili_cases_comb_expanded_grouped,"new_data/rcgp_ili_by_age_detailed.csv")
...
## 2022-23 
rcgp_flu <- read_csv("new_data/rcgp_influenza_pos_2022_2023.csv")
...
## 2024-25 
rcgp_flu <- read_csv("new_data/rcgp_influenza_pos_2024_2025.csv")
...
## 2022/23 data
rcgp_subtype_22_23 <- read_csv("new_data/rcgp_subtype_pos_2022_2023.csv")
...
## Get 2024/25 data
rcgp_subtype_24_25 <- read_csv("new_data/rcgp_subtype_pos_2024_2025.csv")
...
write_csv(final_dataset,"new_data/ili_plus_datasets_by_age.csv")

######################################################
## Third dataset -- Respiratory DataMart cases
######################################################
## Compare to UKHSA influenza case counts England 2009-2025
influenza_cases_eng <- read_csv("new_data/raw/resp_datamart_all_flu.csv")
...
write_csv(influenza_cases_eng,"new_data/resp_datamart_influenza_cases_england.csv")

######################################################
## Fourth dataset -- Respiratory DataMart cases
######################################################
## Compare to WHO FluNet data
flunet_data <- read_csv("new_data/raw/FlunetData_United Kingdom, England_All Sites_for_03 January 2011 to 15 December 2025.csv") %>% select(-1) %>% select(-c(11))
...
write_csv(flunet_data,"new_data/WHO_flunet_cases.csv")
...
## Plots
ggsave("new_figures/figS1.png",figS1,width=8,height=6,units="in",dpi=300)
ggsave("new_figures/figS2.png",figS2,width=8,height=6,units="in",dpi=300)
ggsave("new_figures/figS3.png",figS3,width=8,height=8,units="in",dpi=300)
